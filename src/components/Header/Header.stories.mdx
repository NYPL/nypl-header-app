...
The `Header` component is meant to replicate (and eventually replace) the
current [NYPL Header package](https://github.com/NYPL/dgx-header-component/)
which is used across various NYPL digital products, with Reservoir Design System
...
- has a log in form for patrons to log into NYPL's
  [Catalog](https://borrow.nypl.org) and
  [Research Catalog](https://www.nypl.org/research/research-catalog/),
- has a search form for patrons to search the Catalog and across
...
- Skip navigation - More information about this feature in the following
  section.
- Notification `<aside>` HTML element - This landmark element renders NYPL alert
  ...
  section.
- `<header>` HTML element - This landmark element denotes the top content
  section of the page. There should only be one main `<header>` element on the
  page as a child of the `<body>` element.
- `<nav>` HTML elements - The `Header` component renders one (1) `<nav>` element
  in the mobile view and two (2) `<nav>` elements in the desktop view. Each
  `<nav>` element appropriately contains its own `aria-label` attribute to
  differentiate it from one another.
- `<form>` HTML element - The `<form>` element is used in the search feature.
- Dropdowns buttons with focus-trap feature - The `Header` uses
  `react-focus-lock` to trap (or loop) focus within its various dropdowns: the
  login submenu, the search form, and the mobile navigation submenu. This is to
  make keyboard navigation easier for users. Trapping focus means that when a
  parent element is clicked and its dropdown becomes visible, focus remains
  among its children while the user tabs. The user may close the dropdown and
  escape the focus trap by clicking on the parent element once again, or by
  using the escape key. Note: This is how the original NYPL `Header` was
  implemented and its Reservoir counterpart replicates this behavior. However,
  this is not the _currently recommended_ way to implement focus management in
  dropdowns. According to the [W3C](https://www.w3.org/wiki/MenuButton), up and
  down arrow keys, rather than tab, should loop focus over child elements.
  Tabbing should instead close the dropdown and move focus to the following
  parent. In the future, we hope to change the Reservoir Header to align with
...
It is important to note that the `SkipNavigation` component expects that the
page it is on has a `main` HTML element with an `id` of `mainContent`. This
allows the first link in the skip navigation to function correctly.
...
The DS
[`TemplateAppContainer`](https://nypl.github.io/nypl-design-system/reservoir/v1/?path=/docs/components-page-layout-template--template-app-container-component)
component renders the same `SkipNavigation` component that the `Header`
component renders. When using both the `Header` and the `TemplateAppContainer`
components, set the `renderSkipNavigation` prop in the `TemplateAppContainer`
component to `false`. This will prevent two `SkipNavigation` components from
being rendered.
...
When looking for an item, such as book, ebook, DVD, etc, the best search
location is the Library Catalog. For example, please see the
[Catalog search](https://borrow.nypl.org/search?query=cats&searchType=everything&pageSize=10)
for items about cats. This returns books, microform format items, and DVDs about
cats.
...
input field and two buttons, one for the Catalog and one for the NYPL.org
website. The expected behavior is for the user to fill out the form and click on
a button to search the appropriate location.
...
- a link to the NYPL [Library card](https://www.nypl.org/library-card)
  information
...
- a link to NYPL's [Books/Music/Movies](https://www.nypl.org/books-music-movies)
  page
...
- a link to NYPL's [Visit](https://www.nypl.org/connect/visit) page
...
website navigation links, and the first list of links (1) are now rendered as
buttons in a grid.
...
_thinks_ the patron is still logged in. In order to avoid confusion, the
`Header` component performs an extra step to log the patron out completely.
